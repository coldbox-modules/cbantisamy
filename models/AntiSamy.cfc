...
component singleton threadsafe{
	...
	* @return HTMl data or an instance of org.owasp.validator.html.CleanResults
 	*/
	any function HTMLSanitizer( required HTMLData, string policyFile=variables.defaultPolicy, boolean resultsObject=false ){

		if( engine == "ADOBE" ){
			if( arguments.resultsObject ){
				return isSafeHTML(  arguments.htmlData, len( arguments.policyFile ) ? variables.policies[ arguments.policyFile ] : javacast( "null", 0 ) );
			} else {
				return getSafeHTML(  arguments.htmlData, len( arguments.policyFile ) ? variables.policies[ arguments.policyFile ] : javacast( "null", 0 ) );
			}
		} else {
